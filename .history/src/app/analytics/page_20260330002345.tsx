    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div className="glass-card rounded-xl p-4 border border-(--border)">
          <p className="text-xs text-(--muted)">{language === "ne" ? "कुल" : "Total"}</p>
          <p className="text-2xl font-bold">{total}</p>
        </div>
        <div className="glass-card rounded-xl p-4 border border-(--border)">
          <p className="text-xs text-(--muted)">{language === "ne" ? "सम्पन्न" : "Completed"}</p>
          <p className="text-2xl font-bold">{completed}</p>
        </div>
        <div className="glass-card rounded-xl p-4 border border-(--border)">
          <p className="text-xs text-(--muted)">{language === "ne" ? "प्रगतिमा" : "In Progress"}</p>
          <p className="text-2xl font-bold">{inProgress}</p>
        </div>
        <div className="glass-card rounded-xl p-4 border border-(--border)">
          <p className="text-xs text-(--muted)">{language === "ne" ? "म्याद नाघेको" : "Overdue"}</p>
          <p className="text-2xl font-bold">{overdue}</p>
        </div>
      </div>

      <div className="glass-card rounded-xl p-5 border border-(--border) mb-6">
        <h2 className="font-semibold mb-3">{language === "ne" ? "श्रेणी तुलना" : "Category Comparison"}</h2>
      ...
      <div className="glass-card rounded-xl p-5 border border-(--border)">
        <h2 className="font-semibold mb-3">{language === "ne" ? "म्याद नाघेको ट्र्याकर" : "Overdue Tracker"}</h2>
          ...
          {sampleCommitments
            ...
            .map((item) => (
              <li key={item.id}>
                ...
                >
                  #{item.point_number} {language === "ne" ? item.title_ne : item.title_en}
                </Link>
              </li>
            ))}
        </ul>
