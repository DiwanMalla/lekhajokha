...
export default async function CommitmentDetailPage({
  ...
  const categoryName = language === "ne" ? item.category_name_ne : item.category_name_en;
    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-5">
        <p className="text-sm text-(--muted) mb-2">
          {language === "ne" ? "बुँदा" : "Point"} #{String(item.point_number).padStart(3, "0")}
        </p>
        <h1 className="text-3xl font-bold mb-2">{title}</h1>
        <div className="flex flex-wrap items-center gap-2 text-xs text-(--muted)">
          <span className="px-2 py-1 rounded bg-(--secondary)">{item.status}</span>
          <Link
          ...
          <Link
            href={`/ministry/${toSlug(item.responsible_ministry)}`}
            className="px-2 py-1 rounded bg-(--secondary) hover:bg-(--secondary)"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 space-y-5">
          <div className="glass-card rounded-xl p-5 border border-(--border)">
            <h2 className="font-semibold mb-3">{language === "ne" ? "प्रगति" : "Progress"}</h2>
            ...
            <p className="text-sm text-(--muted)">
              {item.progress_percentage}% {language === "ne" ? "पूरा" : "complete"} • {item.status}
            </p>
          </div>
          ...
          <div className="glass-card rounded-xl p-5 border border-(--border)">
            <h2 className="font-semibold mb-3">
              {language === "ne" ? "समयरेखा / गतिविधि लग" : "Timeline / Activity Log"}
            </h2>
        ...
        <div className="space-y-5">
          <div className="glass-card rounded-xl p-5 border border-(--border)">
            <h2 className="font-semibold mb-3">{language === "ne" ? "सम्पादकीय टिप्पणी" : "Editorial Note"}</h2>
            <p className="text-sm text-(--foreground)">{editorialSummary}</p>
          </div>

          <div className="glass-card rounded-xl p-5 border border-(--border)">
            <h2 className="font-semibold mb-3">{language === "ne" ? "नागरिक खण्ड" : "Citizen Section"}</h2>
            <div className="text-sm text-(--foreground) space-y-2">
              <p>{language === "ne" ? "महत्त्व मत" : "Importance votes"}: {item.importance_score}</p>
              <Link
          ...
          <div className="glass-card rounded-xl p-5 border border-(--border)">
            <h2 className="font-semibold mb-3">{language === "ne" ? "सम्बन्धित प्रतिबद्धताहरू" : "Related Commitments"}</h2>
            <ul className="space-y-2 text-sm">
              {related.map((row) => (
                <li key={row.id}>
                  ...
                  >
                    #{row.point_number} {language === "ne" ? row.title_ne : row.title_en}
                  </Link>
                </li>
              ))}
            </ul>
