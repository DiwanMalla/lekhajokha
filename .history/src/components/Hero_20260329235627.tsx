  ...
  return (
    <div className="flex items-center gap-3 justify-center mt-8">
      {[
        ...
        { value: timeLeft.minutes, label: language === 'ne' ? 'मिनेट' : 'Minutes' },
      ].map((item, i) => (
        <div key={i} className="flex flex-col items-center">
          <div className="glass-card rounded-xl px-5 py-3 min-w-[72px] text-center">
            <span className="text-3xl font-extrabold tabular-nums">{String(item.value).padStart(2, '0')}</span>
          </div>
          <span className="text-xs mt-1.5 font-semibold uppercase tracking-widest text-[var(--muted)]">{item.label}</span>
