    ...
    <section className="py-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-3 mb-4">
        {cards.map((item) => (
          <div
            key={item.label}
            className="glass-card rounded-xl p-4 border border-slate-200"
          >
            <div
              className={`inline-flex p-2 rounded-md bg-slate-100 mb-2 ${item.color}`}
            >
              {item.icon}
            </div>
            ...
            <div className="text-xs text-slate-600 mt-1 uppercase tracking-wide">
              {item.label}
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        <div className="glass-card rounded-xl p-4 border border-slate-200 lg:col-span-2">
          ...
          <div className="w-full h-3 bg-slate-200 rounded-full overflow-hidden">
            <div
              className="h-3 bg-gradient-to-r from-emerald-500 to-teal-500"
        ...
        <div className="glass-card rounded-xl p-4 border border-slate-200">
          ...
          <div className="text-sm text-slate-700 space-y-1">
            ...
            <p>
              Due this week: <strong>{thisWeek}</strong>
            </p>
            <p>
              Due this month: <strong>{thisMonth}</strong>
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="flex gap-3 overflow-x-auto pb-1">
          {latestUpdates.map((item) => (
            <Link
              ...
              className="min-w-[280px] rounded-lg border border-slate-200 p-3 bg-white hover:bg-slate-50 transition-colors"
            >
